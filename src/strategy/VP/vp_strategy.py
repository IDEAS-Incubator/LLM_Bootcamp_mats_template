...
class MultiTimeframeTrade:
    """Represents a single trade in the multi-timeframe Volume Profile strategy"""
    entry_time: datetime
    ...
    vp_poc_at_entry: float
    vp_value_area_high: float
    vp_value_area_low: float
    volume_delta_at_entry: float
...
def add_multi_timeframe_volume_profile(df: pd.DataFrame, period: int = 20, bins: int = 50) -> pd.DataFrame:
    """Volume Profile indicators for multiple timeframes"""
    df = df.copy()
    
    # Calculate Volume Profile components
    df['vp_high'] = df['high'].rolling(window=period).max()
    df['vp_low'] = df['low'].rolling(window=period).min()
    df['vp_range'] = df['vp_high'] - df['vp_low']
    
    # Volume-weighted average price (VWAP)
    df['vwap'] = (df['close'] * df['volume']).rolling(window=period).sum() / df['volume'].rolling(window=period).sum()
    
    # Point of Control (POC) - price level with highest volume
    def calculate_poc(row):
        if pd.isna(row['vp_high']) or pd.isna(row['vp_low']) or row['vp_range'] == 0:
            return np.nan
        
        # Create price bins
        price_bins = np.linspace(row['vp_low'], row['vp_high'], bins)
        volume_profile = []
        
        # Calculate volume for each price level
        for i in range(len(price_bins) - 1):
            price_level = (price_bins[i] + price_bins[i + 1]) / 2
            # Find bars where price is within this range
            mask = (df['low'] <= price_level) & (df['high'] >= price_level)
            volume_at_level = df.loc[mask, 'volume'].sum()
            volume_profile.append((price_level, volume_at_level))
        
        if not volume_profile:
            return np.nan
        
        # Find POC (highest volume level)
        poc_price, _ = max(volume_profile, key=lambda x: x[1])
        return poc_price
    
    df['vp_poc'] = df.apply(calculate_poc, axis=1)
    
    # Value Area (70% of volume)
    def calculate_value_area(row):
        if pd.isna(row['vp_high']) or pd.isna(row['vp_low']) or row['vp_range'] == 0:
        ...
        target_volume = total_volume * 0.7  # 70% of total volume
    ...
    df[['vp_value_area_low', 'vp_value_area_high']] = df.apply(calculate_value_area, axis=1, result_type='expand')
    
    # Price position relative to Volume Profile
    df['price_above_poc'] = df['close'] > df['vp_poc']
    df['price_above_value_area'] = df['close'] > df['vp_value_area_high']
    df['price_below_value_area'] = df['close'] < df['vp_value_area_low']
    
    # Volume Profile strength
    df['vp_strength'] = df['volume'] / df['volume'].rolling(window=period).mean()
    df['vp_strong'] = df['vp_strength'] > VP_THRESHOLD
    
    # Volume Delta (buying vs selling pressure)
    df['volume_delta'] = np.where(df['close'] > df['open'], df['volume'], -df['volume'])
    df['volume_delta_sma'] = df['volume_delta'].rolling(window=period).mean()
    df['volume_delta_strong'] = abs(df['volume_delta']) > VOLUME_DELTA_THRESHOLD * df['volume'].rolling(window=period).mean()
    
    # Volume Profile breakout signals
    df['vp_breakout_up'] = (df['close'] > df['vp_value_area_high']) & (df['volume'] > df['volume'].rolling(window=period).mean() * 1.5)
    df['vp_breakout_down'] = (df['close'] < df['vp_value_area_low']) & (df['volume'] > df['volume'].rolling(window=period).mean() * 1.5)
    
    # Volume Profile rejection signals
    df['vp_rejection_up'] = (df['high'] > df['vp_value_area_high']) & (df['close'] < df['vp_value_area_high']) & (df['volume'] > df['volume'].rolling(window=period).mean())
    df['vp_rejection_down'] = (df['low'] < df['vp_value_area_low']) & (df['close'] > df['vp_value_area_low']) & (df['volume'] > df['volume'].rolling(window=period).mean())
...
def backtest_multi_timeframe_strategy(df: pd.DataFrame, 
    ...
    """
    Multi-timeframe Volume Profile trading strategy
    
    Entry Logic:
    - Volume Profile breakouts above/below value area
    - Price rejection from Volume Profile levels
    ...
    required_cols = ["vp_poc", "vp_value_area_high", "vp_value_area_low", "vp_breakout_up", "vp_breakout_down",
                     "vp_rejection_up", "vp_rejection_down", "vp_strong", "volume_delta_strong",
                     "momentum_confirmation", "volume_confirmation", "price_above_poc", "price_above_value_area"]
    ...
    for idx, (timestamp, row) in enumerate(df.iterrows()):
        ...
        for trade in active_trades:
            ...
            if trade.side == 'long':
                ...
                # Volume Profile reversal exit
                elif row['vp_rejection_down'] and current_pnl > 0.005:
                    exit_reason = "vp_reversal"
                    exit_price = row['close']
                
                # Price below POC exit
                elif not row['price_above_poc'] and current_pnl > 0.003:
                    exit_reason = "vp_breakdown"
                    exit_price = row['close']
                ...
                # Volume Profile reversal exit
                elif row['vp_rejection_up'] and current_pnl > 0.005:
                    exit_reason = "vp_reversal"
                    exit_price = row['close']
                
                # Price above POC exit
                elif row['price_above_poc'] and current_pnl > 0.003:
                    exit_reason = "vp_breakout"
                    exit_price = row['close']
        ...
        if len(active_trades) < 2:  # Max 2 concurrent trades
            ...
            long_conditions = [
                # Volume Profile breakout with confirmation
                row['vp_breakout_up'] and row['vp_strong'] and row['momentum_confirmation'] and row['volume_confirmation'],
                
                # Volume Profile rejection with confirmation
                row['vp_rejection_down'] and row['vp_strong'] and row['momentum_confirmation'] and row['volume_confirmation'],
                
                # Price above POC with volume confirmation
                row['price_above_poc'] and row['volume_delta_strong'] and row['momentum_up'],
                
                # Price above value area with momentum
                row['price_above_value_area'] and row['momentum_confirmation'] and row['volume_confirmation'],
                
                # Bounce from support with VP alignment
                row['near_fib_38'] and row['price_above_poc'] and row['volume_confirmation'],
                
                # Hammer pattern with VP support
                row['hammer'] and row['price_above_poc'] and row['volume_confirmation']
            ]
            
            # Multiple short entry conditions
            short_conditions = [
                # Volume Profile breakout with confirmation
                row['vp_breakout_down'] and row['vp_strong'] and row['momentum_confirmation'] and row['volume_confirmation'],
                
                # Volume Profile rejection with confirmation
                row['vp_rejection_up'] and row['vp_strong'] and row['momentum_confirmation'] and row['volume_confirmation'],
                
                # Price below POC with volume confirmation
                not row['price_above_poc'] and row['volume_delta_strong'] and row['momentum_down'],
                
                # Price below value area with momentum
                row['price_below_value_area'] and row['momentum_confirmation'] and row['volume_confirmation'],
                
                # Rejection from resistance with VP alignment
                row['near_fib_61'] and not row['price_above_poc'] and row['volume_confirmation'],
                
                # Shooting star pattern with VP resistance
                row['shooting_star'] and not row['price_above_poc'] and row['volume_confirmation']
            ]
            ...
            if any(long_conditions):
                ...
                new_trade = MultiTimeframeTrade(
                    ...
                    bars_held=None,
                    exit_reason=None,
                    vp_poc_at_entry=row['vp_poc'],
                    vp_value_area_high=row['vp_value_area_high'],
                    vp_value_area_low=row['vp_value_area_low'],
                    volume_delta_at_entry=row['volume_delta'],
                    price_position_at_entry=row['price_position'],
...
def compute_multi_timeframe_metrics(trades: List[MultiTimeframeTrade], 
    ...
    """Compute performance metrics for multi-timeframe Volume Profile strategy"""
    if not trades:
        return {
...
def run_multi_timeframe_scenario(timeframe: str, 
    ...
    """Run multi-timeframe Volume Profile backtest for a specific scenario"""
    ...
    # Add all indicators
    df = add_multi_timeframe_volume_profile(df, VP_PERIOD, VP_BINS)
...
if __name__ == "__main__":
    print("=" * 80)
    print("MULTI-TIMEFRAME VOLUME PROFILE TRADING STRATEGY BACKTEST")
    print("=" * 80)
    ...
    if all_results:
        print("\n" + "=" * 80)
        print("MULTI-TIMEFRAME VOLUME PROFILE PERFORMANCE SUMMARY")
        print("=" * 80)
        ...
        print("=" * 80)
        print("TOP PERFORMING VOLUME PROFILE COMBINATIONS")
        print("=" * 80)
        ...
        print("=" * 80)
        print("VOLUME PROFILE PERFORMANCE BY TIMEFRAME")
        print("=" * 80)
        ...
        filename = f"vp_multi_timeframe_performance_report_{SYMBOL.replace('/','_')}_{timestamp}.csv"
        df_results.to_csv(filename, index=False)
        print(f"\nComprehensive Volume Profile performance report saved to: {filename}")
        
        # Best recommendation
        if not df_results.empty:
            best_combo = df_results.iloc[0]
            print(f"\n🏆 BEST VOLUME PROFILE RECOMMENDATION:")
