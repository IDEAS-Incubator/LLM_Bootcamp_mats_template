...
class MultiTimeframeTrade:
    """Represents a single trade in the multi-timeframe MACD strategy"""
    entry_time: datetime
...
def add_multi_timeframe_macd(df: pd.DataFrame, fast: int = 12, slow: int = 26, signal: int = 9) -> pd.DataFrame:
    """MACD indicators for multiple timeframes"""
    df = df.copy()
    ...
    df['signal_momentum'] = df['macd_signal'].diff(2)
    
    # MACD strength
    df['macd_strength'] = abs(df['macd_histogram']) > MACD_HISTOGRAM_THRESHOLD
    
    # MACD divergence
    df['macd_divergence_bull'] = (df['close'] < df['close'].shift(1)) & (df['macd'] > df['macd'].shift(1))
    df['macd_divergence_bear'] = (df['close'] > df['close'].shift(1)) & (df['macd'] < df['macd'].shift(1))
...
def backtest_multi_timeframe_strategy(df: pd.DataFrame, 
    ...
    - MACD histogram strength confirmation
    ...
    required_cols = ["macd", "macd_signal", "macd_histogram", "macd_cross_up", "macd_cross_down",
                     "macd_strength", "momentum_confirmation", "volume_confirmation", 
                     "price_above_sma20", "price_above_sma50", "oversold_zone", "overbought_zone"]
    ...
    for idx, (timestamp, row) in enumerate(df.iterrows()):
        ...
        for trade in active_trades:
            ...
            if trade.side == 'long':
                ...
                # MACD reversal exit
                elif row['macd_cross_down'] and current_pnl > 0.005:
                ...
                # MACD reversal exit
                elif row['macd_cross_up'] and current_pnl > 0.005:
                    exit_reason = "macd_reversal"
                    exit_price = row['close']
        ...
        if len(active_trades) < 2:  # Max 2 concurrent trades
            ...
            long_conditions = [
                # Strong MACD crossover with confirmation
                row['macd_cross_up'] and row['macd_strength'] and row['momentum_confirmation'] and row['volume_confirmation'],
                
                # MACD bullish divergence
                row['macd_divergence_bull'] and row['macd_strength'] and row['volume_confirmation'],
                
                # MACD above signal with momentum
                row['macd'] > row['macd_signal'] and row['macd_momentum'] > 0 and row['volume_confirmation'],
                
                # Price above moving averages with MACD support
                row['price_above_sma20'] and row['price_above_sma50'] and row['macd'] > 0 and row['volume_confirmation'],
                
                # Oversold bounce with MACD turning
                row['oversold_zone'] and row['macd_momentum'] > 0 and row['volume_confirmation']
            ]
            
            # Multiple short entry conditions
            short_conditions = [
                # Strong MACD crossover with confirmation
                row['macd_cross_down'] and row['macd_strength'] and row['momentum_confirmation'] and row['volume_confirmation'],
                
                # MACD bearish divergence
                row['macd_divergence_bear'] and row['macd_strength'] and row['volume_confirmation'],
                
                # MACD below signal with momentum
                row['macd'] < row['macd_signal'] and row['macd_momentum'] < 0 and row['volume_confirmation'],
                
                # Price below moving averages with MACD resistance
                not row['price_above_sma20'] and not row['price_above_sma50'] and row['macd'] < 0 and row['volume_confirmation'],
                
                # Overbought rejection with MACD turning
                row['overbought_zone'] and row['macd_momentum'] < 0 and row['volume_confirmation']
            ]
            ...
            if any(long_conditions):
                ...
                new_trade = MultiTimeframeTrade(
                    ...
                    bars_held=None,
                    exit_reason=None,
                    macd_at_entry=row['macd'],
                    signal_at_entry=row['macd_signal'],
                    histogram_at_entry=row['macd_histogram'],
...
def compute_multi_timeframe_metrics(trades: List[MultiTimeframeTrade], 
    ...
    """Compute performance metrics for multi-timeframe MACD strategy"""
    if not trades:
        return {
...
def run_multi_timeframe_scenario(timeframe: str, 
    ...
    """Run multi-timeframe MACD backtest for a specific scenario"""
    ...
    # Add all indicators
    df = add_multi_timeframe_macd(df, MACD_FAST, MACD_SLOW, MACD_SIGNAL)
...
if __name__ == "__main__":
    print("=" * 80)
    print("MULTI-TIMEFRAME MACD TRADING STRATEGY BACKTEST")
    print("=" * 80)
    ...
    if all_results:
        print("\n" + "=" * 80)
        print("MULTI-TIMEFRAME MACD PERFORMANCE SUMMARY")
        print("=" * 80)
        ...
        print("=" * 80)
        print("TOP PERFORMING MACD COMBINATIONS")
        print("=" * 80)
        ...
        print("=" * 80)
        print("MACD PERFORMANCE BY TIMEFRAME")
        print("=" * 80)
        ...
        filename = f"macd_multi_timeframe_performance_report_{SYMBOL.replace('/','_')}_{timestamp}.csv"
        df_results.to_csv(filename, index=False)
        print(f"\nComprehensive MACD performance report saved to: {filename}")
        
        # Best recommendation
        if not df_results.empty:
            best_combo = df_results.iloc[0]
            print(f"\n🏆 BEST MACD RECOMMENDATION:")
